...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_single_row'])) {
   ...
   // Fetch the live hired count (regardless of joining_status)
$sql = "
  SELECT COUNT(*) as count 
  FROM placed_students 
  WHERE company_name = (SELECT company_name FROM drive_data WHERE id = ?)
    AND role_id = (SELECT role_id FROM drive_data WHERE id = ?)
";
...
FROM drive_data d
LEFT JOIN drives drv ON d.drive_id = drv.drive_id
LEFT JOIN drive_roles dr ON d.role_id = dr.role_id
...
while ($row = $result->fetch_assoc()) {
    ...
    // ✅ Update hired_count in DB (minimal fix)
    $updateStmt = $conn->prepare("UPDATE drive_data SET hired_count = ? WHERE id = ?");
    $updateStmt->bind_param("ii", $row['hired_count'], $row['id']);
    $updateStmt->execute();
    $updateStmt->close();
