...
usort($commonFields, function($a, $b) use ($fields, $sectionOrder) {
    // Keep Resume last
    if ($a['name'] === 'Upload Resume (PDF)') return 1;
    if ($b['name'] === 'Upload Resume (PDF)') return -1;
    ...
    $orderA = array_search($a['name'], $fields[$sectionA]);
    $orderB = array_search($b['name'], $fields[$sectionB]);
...
?>
<div class="field-group">
    <label><?= $field['name'] ?> <?= $field['mandatory'] ? '<span style="color:red;">*</span>' : '' ?></label>
    <?php
    if ($preset) {
switch ($preset['type']) {
    case "radio":
