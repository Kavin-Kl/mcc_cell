...
// If it's a comma-separated list, use the first entry (same logic you used in the copy box)
$graduating_year_raw = trim((string)($d['graduating_year'] ?? ''));
if ($graduating_year_raw !== '') {
    $firstYear = explode(',', $graduating_year_raw)[0];
    $gradDisplay = htmlspecialchars($firstYear) . ' grads';
} else {
...
if ($companyName === '') {
    ...
    if ($gradYear !== '') {
        // If the year looks like a list (comma separated) use first value
        $firstYear = explode(',', $gradYear)[0];
        $companyLine = htmlspecialchars($companyName) . " is hiring " . htmlspecialchars($firstYear) . " grads";
    } else {
