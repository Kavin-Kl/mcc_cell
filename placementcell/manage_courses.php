...
function filterByType(type) {
    ...
    document.querySelectorAll('.filter-buttons .filter-btn').forEach(btn => {
        if (btn.textContent.includes(type.toUpperCase()) || (type === 'all' && btn.textContent === 'All')) {
            btn.classList.add('active');
        } else if (!btn.textContent.includes('Only') && !btn.textContent.includes('Active') && !btn.textContent.includes('Inactive')) {
